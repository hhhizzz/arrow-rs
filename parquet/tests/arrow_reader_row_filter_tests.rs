...
fn create_int64_array(size: usize) -> ArrayRef {
    ...
    let unique_index2 = 99999;
    values[unique_index2] = 1234; // Unique value for point lookup
    ...
    fn filter_batch(&self, batch: &RecordBatch) -> arrow::error::Result<BooleanArray> {
        match self {
            ...
            FilterType::PointLookup => {
                let array1 = batch.column(batch.schema().index_of("int64")?);
                let scalar1 = Int64Array::new_scalar(9999);
                let result1 = eq(array1, &scalar1)?;

                let array2 = batch.column(batch.schema().index_of("int64")?);
                let scalar2 = Int64Array::new_scalar(1234);
                let result2 = eq(array2, &scalar2)?;

                Ok(or(&result1, &result2)?)
            }
...
#[tokio::test]
async fn test_in_memory_reader() {
    let parquet_file = Bytes::from(write_parquet_file());
    let reader = InMemoryReader::try_new(&parquet_file).unwrap();
    let metadata = reader.metadata().clone();
    assert_eq!(metadata.file_metadata().num_rows(), 100_000);

    let all_indices = vec![0, 1, 2, 3];
    let filter_col = FilterType::PointLookup.filter_projection().to_vec();
    // For the projection, either select all columns or exclude the filter column(s).
    let output_projection: Vec<usize> = all_indices.clone();
    ...
    let pred_mask = ProjectionMask::roots(schema_descr, filter_col.clone());
    let filter = ArrowPredicateFn::new(pred_mask, move |batch: RecordBatch| {
        Ok(FilterType::PointLookup.filter_batch(&batch).unwrap())
    });
    let row_filter = RowFilter::new(vec![Box::new(filter)]);
    benchmark_async_reader(reader.clone(), ProjectionMask::all(), row_filter).await;
    // }
}
